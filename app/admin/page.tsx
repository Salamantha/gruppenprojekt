...
type DashboardData = {
  ...
  accuracyByAiTrust: Array<{ value: string; total: number; accuracy: Rate }>;
  participants: Array<{
    id: string;
    createdAt: string;
    ...
    <main className="min-h-screen bg-slate-50 text-slate-950">
      <div className="mx-auto max-w-7xl px-4 py-8 md:px-8 md:py-10">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <StatCard
            label="Teilnehmende"
            value={String(data.overview.participants)}
            note={`${data.overview.completedParticipants} abgeschlossen · ${formatRate(data.overview.completionRate)} Completion · ${data.overview.excludedParticipants} ausgeschlossen`}
          />
        ...
        <div className="mt-6">
          <Section title="KI-Vertrauen vs. Accuracy" subtitle="Explorativ: Vertrauen darauf, dass KI-generierte Inhalte inhaltlich stimmen.">
            <div className="space-y-3">
              {data.accuracyByAiTrust.map((row) => (
                <div key={row.value} className="grid grid-cols-[140px_1fr_60px] items-center gap-3 text-sm">
                  <span className="font-semibold">
                    {row.value} <span className="font-normal text-slate-400">(n={row.total})</span>
                  </span>
                  <RateBar value={row.accuracy} />
                  <span className="text-right font-semibold">{formatRate(row.accuracy)}</span>
                </div>
                    ...
                    <tr key={participant.id} className="border-t border-slate-100">
                      ...
                      <td className="px-3 py-3">
                        {participant.excludedAt ? (
                          <span className="font-semibold text-red-700">Ausgeschlossen</span>
                        ) : participant.completedAt ? (
                          <span className="font-semibold text-emerald-700">Abgeschlossen</span>
                        ) : (
