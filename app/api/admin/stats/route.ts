...
export async function GET(request: NextRequest) {
  ...
  const [participants, trials] = await Promise.all([
    prisma.participant.findMany({
      select: {
        ...
        questionnaire: {
          select: {
            selfRatedCookingSkill: true,
            trustInAiAccuracy: true,
          },
        },
      },
  ...
  const accuracyByCookingSkill = [1, 2, 3, 4, 5].map((skill) => {
    const participantIds = new Set(
      participants.filter((participant) => participant.questionnaire?.selfRatedCookingSkill === skill).map((participant) => participant.id)
    );
    const group = evaluable.filter((trial) => participantIds.has(trial.participantId));
    const groupCorrect = group.filter((trial) => trial.isCorrectDetection === true).length;
    return { value: skill, total: group.length, accuracy: pct(groupCorrect, group.length) };
  });

  const accuracyByAiTrust = [1, 2, 3, 4, 5].map((trust) => {
    const participantIds = new Set(
      participants.filter((participant) => participant.questionnaire?.trustInAiAccuracy === trust).map((participant) => participant.id)
    );
    const group = evaluable.filter((trial) => participantIds.has(trial.participantId));
    const groupCorrect = group.filter((trial) => trial.isCorrectDetection === true).length;
