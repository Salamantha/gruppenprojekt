  ...
  return (
    <div
      className={`rounded-lg border transition-colors ${
        flagged ? "bg-red-50 border-red-400" : "bg-white border-gray-200"
      }`}
    >
      <button
        type="button"
      ...
      {flagged && (
        <div className="px-3 pb-3">
          <label className="block text-xs font-semibold text-gray-600 mb-1.5">Warum ist diese Zeile auffällig?</label>
          <select
...
export default function RecipeReviewCard({
  ...
  const missingIngredientItem: FlaggedItem = {
    field: "INGREDIENT",
    index: -1,
    reason: "OMITTED_INGREDIENT",
  };
  const missingStepItem: FlaggedItem = { field: "STEP", index: -1, reason: "OMITTED_STEP" };
      ...
      <section className="mb-8">
        ...
        <div className="space-y-2">
          ...
          {flagging && (
            <ReviewRow
              flagging
              item={missingIngredientItem}
              flaggedItem={getFlaggedItem(flaggedItems, "INGREDIENT", -1)}
              reasonOptions={[{ value: "OMITTED_INGREDIENT", label: "Eine Zutat fehlt" }]}
              onToggleFlag={onToggleFlag}
              onReasonChange={onReasonChange}
            >
      ...
      <section>
        ...
        <div className="space-y-2">
          ...
          {flagging && (
            <ReviewRow
              flagging
              item={missingStepItem}
              flaggedItem={getFlaggedItem(flaggedItems, "STEP", -1)}
              reasonOptions={[{ value: "OMITTED_STEP", label: "Ein Arbeitsschritt fehlt" }]}
              onToggleFlag={onToggleFlag}
              onReasonChange={onReasonChange}
            >
